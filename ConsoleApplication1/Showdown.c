    ...
    if valid card, updates *offset to be just beyond last char of card */
{
    ...
    *offset += (int)(p - (string + *offset));
    if (*p == '1') {
    	...
    	++p;
    	++*offset; }
    if ((i = sscanf_s(p, " %c%c%n", &c, &s, &consumed)) != 2) {
...
Boolean GetNbrUnknown(int *nUnknown)
{
    ...
    *nUnknown = 0;
    ...
    if (max > 2)
        max = 2;
    sprintf_s(prompt, "Number of players with unknown hole cards (0 to %d) [0]: ", max);
    do {
...
Boolean UserConfirm(double nDeals)
{
	...
	sprintf_s(prompt, "%s deals required.  Start dealing? (y/n) [y]: ", WithCommas(nDeals, s));
    while (true) {
        if (!GetResponse(prompt, s, 80,
